...
final class ClamAvAttachmentScanner implements AttachmentScannerInterface, ContentAttachmentScannerInterface
{
    ...
    public function healthCheck(): array
    {
        ...
        if (! is_resource($socket)) {
            return ['healthy' => false, 'reachable' => false, 'protocol' => 'unavailable'];
        }
        try {
            stream_set_timeout($socket, (int) $timeout);
            if (@fwrite($socket, "zPING\0") !== 6) {
                return ['healthy' => false, 'reachable' => true, 'protocol' => 'write_failed'];
            }
            $response = fread($socket, 64);
            $timedOut = stream_get_meta_data($socket)['timed_out'] ?? false;
            if ($timedOut) {
                return ['healthy' => false, 'reachable' => true, 'protocol' => 'timeout'];
            }
    ...
    public function scan(AttachmentScanRequest $request): AttachmentScanResultData
    {
        ...
        if (! is_resource($stream)) {
            return new AttachmentScanResultData(AttachmentScanResult::Failed, scannerVersion: 'clamav:missing');
        }
        ...
        try {
            ...
            return $this->scanStream($stream, $address, $timeout, $maxBytes);
        } catch (\Throwable) {
    ...
    private function scanStream($stream, string $address, float $timeout, int $maxBytes): AttachmentScanResultData
    {
        $socket = @stream_socket_client($address, $errorCode, $errorMessage, $timeout, STREAM_CLIENT_CONNECT);
        if (! is_resource($socket)) {
            return new AttachmentScanResultData(AttachmentScanResult::Failed, scannerVersion: 'clamav:unavailable');
        }
        try {
            ...
            if (! $this->writeAll($socket, "zINSTREAM\0")) {
                return new AttachmentScanResultData(AttachmentScanResult::Failed, scannerVersion: 'clamav:protocol');
            }
            $sent = 0;
            while (! feof($stream)) {
                ...
                if (! $this->writeAll($socket, pack('N', strlen($chunk)).$chunk)) {
                    return new AttachmentScanResultData(AttachmentScanResult::Failed, scannerVersion: 'clamav:write');
                }
            }
            ...
            while (! feof($socket)) {
                ...
                $response .= $part;
                if (str_contains($response, "\0") || str_contains($response, 'OK') || str_contains($response, 'FOUND') || str_contains($response, 'ERROR')) {
                    break;
                }
            }
            if ((stream_get_meta_data($socket)['timed_out'] ?? false) === true) {
                return new AttachmentScanResultData(AttachmentScanResult::Failed, scannerVersion: 'clamav:timeout');
            }
            $response = trim(str_replace("\0", '', $response));
            if (str_ends_with($response, 'OK')) {
                return new AttachmentScanResultData(AttachmentScanResult::Clean, scannerVersion: 'clamav');
            }
            if (str_contains($response, 'FOUND')) {
                $signature = preg_replace('/[^A-Za-z0-9._:+ -]/', '', trim((string) preg_replace('/^.*:\s*(.*?)\s+FOUND.*$/s', '$1', $response))) ?: 'unknown';
