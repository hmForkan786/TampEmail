...
final class CreateMailServerAction
{
    ...
    public function execute(CreateMailServerData $data, MailServerMutationContext $context): MailServer
    {
        return DB::transaction(function () use ($data, $context): MailServer {
            ...
            if (! $actor instanceof User || ! $actor->isPlatformOperator() || ! Gate::forUser($actor)->allows('create', MailServer::class)) {
                throw new AuthorizationException('Mail-server creation is not allowed.');
            }
    ...
    public static function safeValues(MailServer $server): array
    {
        return [
            ...
            'port' => $server->metadata['port'] ?? null,
            'provider' => $server->provider,
            'protocol' => $server->protocol,
            'pool_key' => $server->pool_key,
            'max_inboxes' => $server->max_inboxes,
            'max_throughput' => $server->max_throughput,
            'is_active' => $server->is_active,
            'operational_status' => $server->operationalStatusEnum()->value,
    ...
    /** @return array<string,mixed> */
    public static function metadata(MailServerMutationContext $context, CarbonInterface $at): array
    {
