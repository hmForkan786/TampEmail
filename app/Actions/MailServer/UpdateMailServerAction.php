...
final class UpdateMailServerAction
{
    ...
    public function execute(MailServer $mailServer, UpdateMailServerData $data): MailServer
    {
        $data = $data->withProvisioningFields(
            $data->poolKey === null ? null : MailServerProvisioningInvariant::poolKey($data->poolKey),
            $data->maxInboxes === null ? null : MailServerProvisioningInvariant::maxInboxes($data->maxInboxes),
        );
