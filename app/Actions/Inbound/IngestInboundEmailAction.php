...
final class IngestInboundEmailAction
{
    ...
    {
        $storedPaths = []; $started = microtime(true); $scanAttachmentIds = [];
        try {
            $email = DB::transaction(function () use ($parsed, $resolution, &$storedPaths, $started, &$scanAttachmentIds): Email {
                ...
                foreach ($parsed->attachments as $attachment) {
                    ...
                    if (config('attachments.scanner_backend', 'disabled') !== 'disabled' && Storage::disk('attachments')->exists($path)) $scanAttachmentIds[] = (string) $record->id;
                }
            ...
            foreach ($storedPaths as $path) Storage::disk('attachments')->delete($path);
