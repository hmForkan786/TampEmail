...
final class IngestInboundEmailAction
{
    public function __construct(
        private readonly InboundHtmlSanitizer $sanitizer,
        private readonly InboundMetricsRecorder $metrics,
        private readonly AttachmentScanService $scanService,
    ) {}
    ...
    public function execute(ParsedInboundEmail $parsed, InboundResolution $resolution): Email
    {
        ...
        try {
            $email = DB::transaction(function () use ($parsed, $resolution, &$storedPaths, $started, &$scanAttachmentIds, &$skipAttachmentIds): Email {
                ...
                $maxCount = (int) config('attachments.max_count', 20);
                $maxBytes = (int) config('attachments.max_bytes', 26214400);
                $maxTotal = (int) config('attachments.max_total_bytes', 52428800);
                if (count($parsed->attachments) > $maxCount || array_sum(array_map(fn ($a): int => $a->sizeBytes, $parsed->attachments)) > $maxTotal || collect($parsed->attachments)->contains(fn ($a): bool => $a->sizeBytes > $maxBytes)) {
                    throw new \InvalidArgumentException('Attachment limits exceeded.');
                }
                ...
                $scannerEnabled = config('attachments.scanner_backend', 'disabled') !== 'disabled';
                foreach ($parsed->attachments as $attachment) {
                    $path = 'quarantine/'.$email->id.'/'.bin2hex(random_bytes(16));
                    Storage::disk('attachments')->put($path, $attachment->content);
                    $storedPaths[] = $path;
                    ...
                    if ($scannerEnabled && Storage::disk('attachments')->exists($path)) {
                        $scanAttachmentIds[] = (string) $record->id;
                    } elseif (! $scannerEnabled) {
                        $skipAttachmentIds[] = (string) $record->id;
                    }
                }
            ...
            $this->metrics->record(null, 'failed', ProcessingStage::StoreBody, ProcessingLogStatus::Failed, ['transaction' => 'rolled_back']);
            throw $e;
        }

        foreach ($skipAttachmentIds as $attachmentId) {
            $attachment = Attachment::query()->find($attachmentId);
            if ($attachment !== null) {
                $this->scanService->markSkipped($attachment, 'scanner_disabled');
            }
        }
