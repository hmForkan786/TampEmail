...
{
    public function __construct(private readonly InboundWebhookDispatcher $dispatcher) {}
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if (! RateLimiter::attempt('inbound:'.$provider.':'.$request->ip(), (int) config('inbound.rate_limit_per_minute', 60), fn () => true, 60)) return ApiErrorResponse::make('rate_limit_exceeded', 'Too many inbound requests.', 429);
        ...
        try { $this->dispatcher->dispatch($envelope); } catch (\Throwable) { return ApiErrorResponse::make('dispatch_unavailable', 'Inbound processing is temporarily unavailable.', 503); }
