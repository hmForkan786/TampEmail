...
final class MailServerController extends Controller
{
    ...
    public function store(CreateMailServerRequest $request): JsonResponse
    {
        return (new MailServerResource(
            $this->mailServerService->create(CreateMailServerData::fromArray($request->validated()))
    ...
    {
        return new MailServerResource(
            $this->mailServerService->update($mailServer, UpdateMailServerData::fromArray($request->validated()))
        );
    }
}
