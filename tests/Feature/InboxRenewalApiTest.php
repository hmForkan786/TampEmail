...
function renewalFixture(?User $owner = null, array $inbox = []): array
{
    ...
    ensureCommercialApiAccess($owner, ['inboxes:read', 'inboxes:write']);
...
beforeEach(function (): void {
    config(['api.key_hash_secret' => 'renewal-test-secret', 'inbox_lifetime.renewal_enabled' => true, 'inbox_lifetime.max_extension_hours_per_request' => 24, 'inbox_lifetime.max_absolute_lifetime_hours' => 720]);
});
...
it('rejects foreign anonymous expired inactive deleted and invalid renewal requests', function (): void {
    ...
    $deleted = renewalFixture($owner)[2];
    $deleted->delete();
...
it('rolls back expiry when audit writing fails', function (): void {
    ...
    app()->instance(AuditLogWriter::class, $audit);
