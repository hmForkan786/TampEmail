...
it('keeps terminal failed and infected attachments from automatic rescans', function (): void {
    ...
    $email = Email::query()->create(['inbox_id' => rollbackFixture()[1]->inboxId, 'message_id' => 'state-'.bin2hex(random_bytes(3)), 'sender_email' => 'a@test', 'recipient_email' => 'b@test', 'received_at' => now(), 'size_bytes' => 1, 'processing_status' => 'received']);
    foreach ([AttachmentScanStatus::Failed, AttachmentScanStatus::Infected, AttachmentScanStatus::Clean] as $status) {
        ...
        $result = app(AttachmentScanService::class)->scan($a);
...
it('records retry exhaustion as failed with safe processing metadata', function (): void {
    ...
    $attachment->refresh();
    $log = EmailProcessingLog::query()->where('email_id', $email->id)->latest('created_at')->first();
...
it('treats scanner exceptions as retryable failed state without unsafe details', function (): void {
    ...
    app()->instance(AttachmentScannerInterface::class, new class implements AttachmentScannerInterface
    {
        ...
        {
            throw new RuntimeException('private scanner command');
        }
    });
