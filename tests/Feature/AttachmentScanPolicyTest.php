...
it('marks disabled scanner attachments skipped and blocks visibility', function (): void {
    ...
    expect($result->scan_status)->toBe(AttachmentScanStatus::Skipped)->and($result->is_safe)->toBeNull()->and((new AttachmentVisibilityPolicy)->view(null, $result))->toBeFalse();
});
...
it('marks scanner errors failed and infected attachments unsafe', function (): void {
    ...
    app()->instance(AttachmentScannerInterface::class, new class implements AttachmentScannerInterface
    {
        public function scan(AttachmentScanRequest $request): AttachmentScanResultData
        {
            return new AttachmentScanResultData(AttachmentScanResult::Infected, 'test-signature');
        }
    });
    $result = app(AttachmentScanService::class)->scan($attachment);
