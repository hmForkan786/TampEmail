...
it('dispatches pending attachment scans after commit when enabled', function (): void {
    ...
    Queue::fake();
    [$parsed,$resolution] = dispatchFixture();
...
it('does not dispatch scans when backend is disabled and marks attachments skipped', function (): void {
    ...
    expect($email->attachments()->first()->scan_status)->toBe(AttachmentScanStatus::Skipped)
        ->and($email->attachments()->first()->is_safe)->toBeNull();
});
...
it('keeps ingestion successful without scanner completion', function (): void {
    ...
    [$parsed,$resolution] = dispatchFixture();
    $email = app(IngestInboundEmailAction::class)->execute($parsed, $resolution);
    expect($email->exists)->toBeTrue()->and($email->attachments()->first()->scan_status)->toBe(AttachmentScanStatus::Skipped);
});
...
it('does not dispatch a second job for an already clean attachment', function (): void {
    ...
    $attachment->update(['scan_status' => 'clean', 'is_safe' => true]);
